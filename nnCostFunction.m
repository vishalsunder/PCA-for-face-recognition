...
r1=Theta1(:,(2:input_layer_size+1));
r2=Theta2(:,(2:hidden_layer_size+1));
J = -(1/m)*(sum(sum(Y.*log(a_3)+(1-Y).*log(1-a_3))));
J = -(1/m)*(sum(sum(Y.*log(a_3)+(1-Y).*log(1-a_3))))+(lambda/(2*m))*(r1(:)'*r1(:)+r2(:)'*r2(:));
for t=1:m
    Y=(1:num_labels)';
    a_1=X(t,:)';
    ...
    Y=(Y==y(t));
    delta_3=a_3-Y;
    delta_2=(Theta2'*delta_3).*sigmoidGradient([0;Theta1*a_1]);
